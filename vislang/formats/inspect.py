...
from vislang.formats.adapters import (get_adapter, detect_positions,  # noqa: F401
...
def _remote_conn(uri):
    ...
    from vislang.remote.download import establish_connection, _parse_remote
    from vislang.remote.reduce import _normalize_remote
    try:
...
def _run_remote_inspect(conn, remote_path):
    ...
    import shlex
    from vislang.remote.download import run_remote
    from vislang.remote.reduce import _parse_meta
    from vislang.remote.hosts import remote_python, remote_repo
...
def _inspect_via_fetch(uri, positions):
    ...
    from vislang.interpreter.planner import _fetch_remote
...
def remote_is_dir(uri):
    ...
    from vislang.remote.download import remote_probe
    conn, remote_path = _remote_conn(uri)
    if conn is None:
        return False
