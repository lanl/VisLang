...
class NeedsAdapterError(Exception):
    """Raised when no adapter recognizes a file and one must be generated by the
    session model. Carries the file path and the format `evidence` (extension,
    size, hex head/tail) that the MCP `inspect` tool hands to the model so it can
    write a reader module and submit it via `submit_adapter`. The run path
    (planner / remote reducer) has no model in the loop, so there this surfaces
    as a clear "inspect the file first to generate a reader" error."""

    def __init__(self, filepath, evidence=None):
        self.filepath = filepath
        self.evidence = evidence
        super().__init__(
            f"No reader recognized {os.path.basename(filepath)!r}; a format "
            f"adapter must be generated. Inspect this file in a session so the "
            f"model can write one.")
...
class FormatAdapter:
    ...
    selection_info) lives in the universal load() in load.py — adapters do
    NOT implement load.
...
class HDF5Adapter(FormatAdapter):
    ...
    def inspect(self, filepath):
        ...
        # not. Try the binding path — it reuses a FROZEN, verified binding for
        # this schema. Fall back to a flat generic listing if none is cached; a
        # new binding is created only via the submit_binding handshake (the
        # session model proposes, verify_binding validates). No API is involved.
        #   VISLANG_NO_BINDING=1  -> always the generic listing (ignore bindings)
        # (bind_hdf5 is frozen-or-None regardless; the remote reducer relies on
        # that to stay model-free yet name-consistent with the local planner.)
        ...
        try:
            ...
            pass  # h5py not available — expected; use generic listing
...
def _llm_module():
    """The Tier-1 module (llm_adapter), imported lazily to avoid a circular
    import (llm_adapter imports from adapters). None only if it fails to import.
    No API/dspy dependency anymore — it holds the evidence gatherer, the
    conformance harness, and the frozen-adapter loader."""
    try:
        from vislang.formats import llm_adapter
        return llm_adapter
    except Exception:
        return None
...
def get_adapter(filepath):
    ...
    if os.path.isdir(filepath):
        raise UnsupportedFormatError(
            f"{filepath!r} is a directory. A folder source is a timeseries handled "
            f"per-timestep by the planner; a single file is expected here.")
    ...
    # Tier 1 — adapters already learned and frozen (no model call)
    _ensure_generated_loaded()
    for adapter in _GENERATED:
        if adapter.can_handle(filepath):
            return adapter

    # Nothing recognized it. We do NOT call a model from inside get_adapter —
    # that would need a synchronous LLM here, and the run path has no model in
    # the loop. Instead gather format evidence and raise NeedsAdapterError; the
    # MCP `inspect` tool surfaces it to the session model, which writes a reader
    # and submits it via `submit_adapter` (llm_adapter.conform_and_freeze).
    mod = _llm_module()
    evidence = None
    if mod is not None:
        try:
            evidence = mod.gather_adapter_evidence(filepath)
        except Exception:
            evidence = None
    raise NeedsAdapterError(filepath, evidence)
