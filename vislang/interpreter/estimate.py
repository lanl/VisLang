...
The numbers below mirror render.py so the estimate matches what render ships:
...
from vislang.formats.inspect import inspect_source, is_remote
...
def _remote_on_disk_mb(uri):
    ...
    try:
        from vislang.formats.inspect import _remote_conn
        from vislang.remote.download import remote_stat
...
def _read_set(info, narrowing):
    ...
    if narrowing is not None and narrowing.project is not None:
        ...
        try:
            from vislang.interpreter.narrowing import post_op_read_vars
