...
# Point-cloud look. These are the difference between "a plot" and something you
# can actually read: a near-black ground so emissive particles carry the image,
# and a point small enough that a dense core resolves into structure instead of
# smearing into one blob. extent/900 keeps a cluster grainy at its center while
# still leaving isolated particles visible.
_BG = 0x05070D            # near-black, faint blue cast
_LABEL = 0x9AA5B8         # axis labels legible on _BG without competing
_POINT_SIZE_DIV = 900.0   # point_size = extent / this
_POINT_WARN = 1_500_000   # beyond this the browser starts to crawl
...
# ---------------------------------------------------------------------------
def _local_density(pts, bounds, bins=_DENSITY_GRID):
    """log10 density at each point, plus the histogram the volume layer uses.

    One 3-D histogram serves both: the volume layer renders it directly, and
    each point looks its own cell up to get a scalar. That scalar is what makes
    a point cloud readable — without it every particle draws the same color and
    the core, where all the structure is, is a flat silhouette.
    """
    hist, _ = np.histogramdd(pts, bins=bins, range=bounds)
    idx = []
    for a in range(3):
        lo, hi = bounds[a]
        j = ((pts[:, a] - lo) / ((hi - lo) or 1.0) * bins).astype(np.int32)
        idx.append(np.clip(j, 0, bins - 1, out=j))
    at_point = hist[idx[0], idx[1], idx[2]].astype(np.float32)
    return np.log10(at_point + 1.0), hist
...
def render_points(dataset_info, cmap=None, opacity=None):
    ...
    if len(pts) > _POINT_WARN:
        print(f"[render] {len(pts):,} points — the browser will be sluggish; "
              f"subsample() or tighten region() to stay under ~{_POINT_WARN:,}.")

    opacity_fn = _OPACITY_FN if opacity is None else np.asarray(opacity, np.float32)
    bounds = [[float(pts[:, a].min()), float(pts[:, a].max())] for a in range(3)]
    extent = max(b[1] - b[0] for b in bounds) or 1.0

    # Dark ground: these clouds are emissive — bright particles reading as light
    # against empty space. On k3d's default white the faint outskirts vanish and
    # an uncolored cloud (white points) disappears completely.
    plot = k3d.plot(height=900, background_color=_BG, grid_visible=False,
                    camera_auto_fit=True, axes_helper=0,
                    label_color=_LABEL, menu_visibility=True)

    dens_at_point, hist = _local_density(pts, bounds)
    ...
    volume = k3d.volume(
        dens,
        color_map=_resolve_cmap(cmap) if cmap is not None else _k3d_colormap('inferno'),
    ...
    volume.visible = False      # a soft halo BEHIND the cloud; opt-in, see below
    plot += volume
    ...
    #
    # 'density' leads: it is always available (derived, never read from disk) and
    # it is the layer that makes structure legible, so it is what you want on
    # screen before you go looking for a specific field.
    layers = [("density", dens_at_point)]
    order = (["mass"] if "mass" in scalar_vars else []) + \
            [v for v in scalar_vars if v != "mass"]
    layers += [(v, np.asarray(data[v]).astype(np.float32)) for v in order]

    point_size = extent / _POINT_SIZE_DIV
    for i, (var, attr) in enumerate(layers):
        ...
        ramp = _resolve_cmap(cmap) if (cmap is not None and var == "density") \
            else _k3d_colormap('inferno' if var == "density" else 'plasma')
        layer = k3d.points(positions=pts, point_size=point_size,
                           shader="flat", attribute=attr, color_map=ramp,
