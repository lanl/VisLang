...
from vislang.dsl.forms import source, threshold, timesteps, save
...
def read_gio(path):
    from vislang.formats.adapters import GenericIOAdapter
...
def test_resolve():
    check("resolve_genericio_preserved_no_extension",
          save_mod._resolve("out", "GenericIO") == ("genericio", "out", False))
    check("resolve_explicit_gio_wins",
          save_mod._resolve("out.gio", "HDF5") == ("genericio", "out.gio", True))
    check("resolve_explicit_hdf5_beats_genericio_source",
          save_mod._resolve("out.hdf5", "GenericIO") == ("hdf5", "out.hdf5", True))
    check("resolve_unknown_source_falls_back_to_npz",
          save_mod._resolve("out", "FITS") == ("npz", "out.npz", False))
...
def test_blockers():
    ...
    out = save_mod.save_loaded(grid, os.path.join(TMP, "grid_degrade"))
    ...
    try:
        save_mod.save_loaded(grid, os.path.join(TMP, "grid.gio"))
...
def test_other_formats():
    ...
    out = save_mod.save_loaded(loaded_info("HDF5", grid), os.path.join(TMP, "h5_pres"))
    check("hdf5_source_preserved", out.endswith(".hdf5"))
    with h5py.File(out, "r") as f:
        check("hdf5_source_data_kept", np.array_equal(f["rho"][...], grid["rho"]))

    out = save_mod.save_loaded(loaded_info("npz", grid), os.path.join(TMP, "npz_pres"))
    check("npz_source_preserved", out.endswith(".npz"))

    out = save_mod.save_loaded(loaded_info("FITS", grid), os.path.join(TMP, "fits_fallback"))
...
# ---------------------------------------------------------------------------
def test_roundtrip():
    reason = save_mod._pygio_write_reason()
    if reason:
    ...
    check("inspect_captures_phys_scale",
          save_mod._phys3(info.attributes.get("phys_scale")) == [64.0, 64.0, 64.0],
          str(info.attributes))
    ...
    check("roundtrip_phys_scale_preserved",
          save_mod._phys3(back_info.attributes.get("phys_scale")) == [64.0, 64.0, 64.0],
